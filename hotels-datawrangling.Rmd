...
```
2 adults is the most common booking. I am rather surprised that there are 403 cases of 0 adults.
...
```{r frequency-adults-canceled}
hotels %>%
  filter(is_canceled == "1") %>%
  count(adults, sort = TRUE)
```

```{r frequency-adults-not-canceled}
hotels %>%
  filter(is_canceled == "0") %>%
  count(adults, sort = TRUE)
```
About a quarter of the bookings with 0 adults were cancelled, so they were probably accidents. 
...
The city hotels have higher daily rate, on average.
